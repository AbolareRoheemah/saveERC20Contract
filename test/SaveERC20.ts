...
describe("SaveERC20", function () {
  ...
  describe("Deposit", function () {
    ...
    // it("should revert on zero address calling", async function () {
    //   const { saveErc20, otherAccount, token } = await loadFixture(deploySaveERC20);

    //   const depositAmount = ethers.parseUnits("10", 18);

    //   expect(await saveErc20.connect(ethers.ZeroAddress).deposit(depositAmount)).to.be.revertedWith("zero addres detected")
    // })
  });
  ...
  describe("Withdraw", function () {
    it("Should deposit successfully", async function () {
      ...
      expect(await token.balanceOf(otherAccount)).to.be.equal(balAfterWithdrawal);
    });
  });
});
